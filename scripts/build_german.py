#!/usr/bin/env python3
"""Build the German sentence array and paste-ready Flow prompt collections."""
...
def load_lessons():
    ...
    originals = []
    topic = None
    for line in (GERMAN / "authoring.txt").read_text(encoding="utf-8").splitlines():
        if not line.strip():
            continue
        if line.startswith("# "):
    ...
    supports = [line.split("|") for line in rows(GERMAN / "bangla-support.txt")]
    ...
    for number, (original, support) in enumerate(zip(originals, supports), 1):
        ...
        assert [de1, de2] == original.pop("originals"), (number, "German source mismatch")
        ...
        full_title = f"জার্মান শিখুন: {title}"
    ...
    all_sentences = [s["german"] for lesson in lessons for s in lesson["sentences"]]
...
def full_prompt(lesson):
    return format_prompt(lesson, "German", "german")
...
def main():
    ...
    for batch, start in enumerate((0, 125), 1):
        ...
        text = [f"# জার্মান ভিডিও {start + 1:03d}–{start + 125:03d}",
                "প্রতিটি ভিডিও আলাদা ১০ সেকেন্ডের পাঠ: দুটি জার্মান বাক্য, ক্যাপশন ছাড়া শুধু কথায় শেখানো হবে।",
    ...
    for lesson in lessons:
        ...
        for sentence in lesson["sentences"]:
            assert sentence["german"] in prompt
    for path in GERMAN.glob("*.json"):
        json.loads(path.read_text(encoding="utf-8"))
    for part in (1, 2):
        md = (GERMAN / f"flow-prompts-{part:02d}.md").read_text(encoding="utf-8")
        assert md.count("```text\n") == 125
        assert md.count("\n```\n") == 125

    levels = Counter(v["approximate_level"] for v in lessons)
    print(f"Validated {len(sentences)} unique German sentences, {len(prompts)} complete prompts, "
    ...
    print("Longest complete lesson speech:", max(
        sum(len(s['german'].split()) for s in v['sentences'])
